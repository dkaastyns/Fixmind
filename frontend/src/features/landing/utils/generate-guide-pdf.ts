 ...
 * dengan narasi bahasa yang ramah, jelas, dan rendering vector murni tanpa bug encoding karakter.
 */
export async function generateUserGuidePDF() {
  ...
  doc.rect(0, 56, pageWidth, 3.5, 'F')

  // Kop Instansi
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(10.5)
  doc.setTextColor(primaryGold[0], primaryGold[1], primaryGold[2])
  doc.text('DEWAN PERWAKILAN RAKYAT DAERAH KOTA SEMARANG', pageWidth / 2, 19, { align: 'center' })

  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8.5)
  doc.setTextColor(240, 240, 240)
  doc.text('Jaringan Dokumentasi dan Informasi Hukum (JDIH) & Bagian Sarana Prasarana', pageWidth / 2, 26, { align: 'center' })

  // Nama Platform
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(18)
  doc.setTextColor(255, 255, 255)
  doc.text('ASETKITA SEMARANG', pageWidth / 2, 41, { align: 'center' })
  ...
  doc.setFontSize(9)
  doc.setTextColor(primaryDark[0], primaryDark[1], primaryDark[2])
  doc.text('INFORMASI PENTING DOKUMEN:', margin + 6, currentY + 7.5)

  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8.2)
  doc.setTextColor(secondarySlate[0], secondarySlate[1], secondarySlate[2])
  doc.text('- Sasaran Pembaca : Seluruh Pegawai, Anggota Dewan, & Admin Sekretariat DPRD Kota Semarang', margin + 6, currentY + 13.5)
  doc.text('- Tujuan Panduan  : Mempermudah pelaporan fasilitas rusak agar segera ditangani teknisi', margin + 6, currentY + 19.5)
  doc.text('- Versi Sistem    : Edisi 2.0 (Dilengkapi Asisten Cerdas AI Priority & Pelaporan Offline)', margin + 6, currentY + 25.5)
  ...
  currentY += splitBab1.length * 3.8 + 2.5
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['3. Pasang di Layar HP (PWA)', 'Di Android (Chrome): Ketuk titik tiga di kanan atas > pilih "Tambahkan ke Layar Utama". Di iPhone (Safari): Ketuk ikon Bagikan > pilih "Add to Home Screen". Ikon ASETKITA akan langsung muncul di layar HP Anda.'],
    ...
    bodyStyles: { fontSize: 7.6, textColor: [30, 41, 59] },
  ...
  currentY += splitBab2.length * 3.8 + 2.5

  autoTable(doc, {
    startY: currentY,
    head: [['Tahap', 'Apa yang Harus Dilakukan?', 'Contoh & Tips Penting']],
    body: [
      ['1. Tekan Tombol Lapor', 'Buka menu "Laporan Masalah" di menu sebelah kiri, lalu tekan tombol kuning "+ Buat Laporan" di pojok kanan atas.', 'Tombol kuning emas berada di bagian atas daftar laporan.'],
      ...
      ['4. Ambil Foto & Kirim', 'Ambil foto bagian yang rusak dengan kamera HP Anda, lalu tekan tombol "Kirim Laporan".', 'Sertakan foto yang jelas dan terang agar teknisi membawa alat yang pas.'],
    ],
    theme: 'grid',
  ...
  currentY += splitBab3.length * 3.8 + 2.5
  ...
  currentY += splitBab4.length * 3.8 + 2.5
  ...
  autoTable(doc, {
    ...
    didDrawCell: (data) => {
      // Gambar lingkaran warna asli vector tanpa bug emoji
      if (data.section === 'body' && data.column.index === 0) {
        ...
        if (col) {
          ...
          doc.circle(data.cell.x + 3.5, data.cell.y + data.cell.height / 2, 2, 'F')
        }
      }
    },
    margin: { left: margin, right: margin },
