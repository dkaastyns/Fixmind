# ASETKITA Semarang — Dokumentasi Logging & Monitoring

Dokumen ini menjelaskan strategi logging (pencatatan log aktivitas/kesalahan) dan rekomendasi monitoring (pemantauan) untuk aplikasi ASETKITA Semarang di lingkungan pengembangan (local) maupun produksi.

---

## Arsitektur Logging

ASETKITA Semarang menggunakan logging terpusat berbasis bawaan NestJS [Logger](https://docs.nestjs.com/techniques/logger) untuk backend. Seluruh log diarahkan ke `stdout` (Standard Output) dan `stderr` (Standard Error) secara default. Hal ini mengikuti prinsip *Twelve-Factor App* di mana log diperlakukan sebagai event stream.
...
Semua error yang terjadi di HTTP layer ditangani secara global oleh [AllExceptionsFilter](file:///d:/ASETKITA Semarang/backend/src/common/filters/all-exceptions.filter.ts).
...
Untuk memantau biaya penggunaan API Gemini, skema database ASETKITA Semarang menyediakan tabel [ai_usage_logs](file:///d:/ASETKITA Semarang/docs/DATABASE.md#ai_usage_logs). Tabel ini dirancang untuk mencatat setiap transaksi LLM:
