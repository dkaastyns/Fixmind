...
export class VirusScannerService {
  ...
  async scanFile(buffer: Buffer, filename: string): Promise<void> {
    ...
    const clamavPort = Number(
      this.config.get<string | number>('CLAMAV_PORT') ?? 3310,
    );
    const vtApiKey = this.config.get<string>('VIRUSTOTAL_API_KEY');

    if (clamavHost) {
      this.logger.log(
        `Scanning file ${filename} with ClamAV at ${clamavHost}:${clamavPort}`,
      );
      const isInfected = await this.scanClamAV(buffer, clamavHost, clamavPort);
      if (isInfected) {
        throw new BadRequestException(
          'Berkas ditolak karena terdeteksi mengandung virus atau malware oleh ClamAV.',
        );
      }
      return;
    }
    ...
    if (vtApiKey) {
      ...
      if (isInfected) {
        throw new BadRequestException(
          'Berkas ditolak karena terdeteksi mengandung virus atau malware oleh VirusTotal.',
        );
      }
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (stats && (stats.malicious > 0 || stats.suspicious > 0)) {
          this.logger.warn(
            `Hash found in VirusTotal registry with malicious/suspicious stats: ${JSON.stringify(stats)}`,
          );
          return true;
        }
      ...
      if (res.status === 404) {
        this.logger.log(
          'Hash not found in registry. Uploading file to VirusTotal...',
        );
        ...
        if (!uploadRes.ok) {
          ...
          this.logger.error(
            `VirusTotal upload failed: ${uploadRes.status} ${errMsg}`,
          );
          return false;
        }

        this.logger.log(
          'File successfully uploaded to VirusTotal for analysis.',
        );
